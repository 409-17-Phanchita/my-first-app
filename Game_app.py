...
if not st.session_state.game_started:
    if not st.session_state.game_finished:
        if st.button("▶️ เริ่มเกม"):
            start_game()  # แก้ไขจาก reset_game() เป็น start_game()
    else:
...
if st.session_state.game_started and not st.session_state.game_finished:
    ...
    ans1 = st.text_input("คำตอบข้อ 1", value=st.session_state.ans1_val)
    st.session_state.ans1_val = ans1

    # ข้อ 2
    st.subheader("ข้อ 2")
    st.write("สัญลักษณ์ **Ge** เลขอะตอม **32** คือธาตุอะไร?")
    ans2 = st.text_input("คำตอบข้อ 2", value=st.session_state.ans2_val)
    st.session_state.ans2_val = ans2

    # ข้อ 3
    st.subheader("ข้อ 3")
    st.write("สัญลักษณ์ **Eu** เลขอะตอม **63** คือธาตุอะไร?")
    ans3 = st.text_input("คำตอบข้อ 3", value=st.session_state.ans3_val)
    st.session_state.ans3_val = ans3

    # ข้อ 4
    st.subheader("ข้อ 4")
    st.write("สัญลักษณ์ **W** เลขอะตอม **74** คือธาตุอะไร?")
    ans4 = st.text_input("คำตอบข้อ 4", value=st.session_state.ans4_val)
    st.session_state.ans4_val = ans4

    # ข้อ 5
    st.subheader("ข้อ 5")
    st.write("สัญลักษณ์ **Fm** เลขอะตอม **100** คือธาตุอะไร?")
    ans5 = st.text_input("คำตอบข้อ 5", value=st.session_state.ans5_val)
...
if st.session_state.game_finished:
    ...
    # เกณฑ์ประเมินผู้เล่น
    if score == 5:
        st.success("อย่างโหดเลยคร้าบจารย์! ตอบถูกทั้ง 5 ข้อ")
    elif score == 4:
        st.info("So Very Good but พยายามอีกนิสส์")
    elif score == 3:
        st.info("พี่ทำได้มากกว่านี้แน่นอน!")
    elif score == 2:
        st.warning("พยายามกว่านี้นะะ")
    elif score == 1:
        st.warning("สู้ๆละคนดีของพี่")
    elif score == 0:
        st.warning("พยายามอีกนิสนะน้อง💪🏻")
